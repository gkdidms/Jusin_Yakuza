...
CUISkillDestroyer::CUISkillDestroyer(const CUISkillDestroyer& rhs)
    :CUIScene{ rhs }
{
}
...
HRESULT CUISkillDestroyer::Add_UIData(CUI_Object* pUIObject, wstring wstrPrototypeTag)
{
	if (pUIObject->Get_Event())
	{
		...
		else if(m_EventUI.empty() || m_EventUI.size()<8)
...
HRESULT CUISkillDestroyer::Show_Scene()
{
	__super::Show_Scene();
...
HRESULT CUISkillDestroyer::Close_Scene()
{
	if (m_isGet)
	{
...
HRESULT CUISkillDestroyer::Initialize(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, void* pArg)
{
...
HRESULT CUISkillDestroyer::Tick(const _float& fTimeDelta)
{
...
HRESULT CUISkillDestroyer::Late_Tick(const _float& fTimeDelta)
{
	for (size_t i = 0; i < m_UI.size(); i++)
	{
		if (m_fGetTime > 0.f)
...
_bool CUISkillDestroyer::Click_InterSect()
{
	for (size_t i = 0; i < m_pBall.size(); i++)
	{
...
_bool CUISkillDestroyer::Over_InterSect()
{
	for (size_t i = 0; i < m_pBall.size(); i++)
	{
		if (m_pBall[i]->Click_Intersect(0))
...
void CUISkillDestroyer::OverAction()
{
...
void CUISkillDestroyer::Set_Money(const _float& fTimeDelta)
{
...
CUISkillDestroyer* CUISkillDestroyer::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, void* pArg)
{
	CUISkillDestroyer* pInstance = new CUISkillDestroyer();
	if (FAILED(pInstance->Initialize(pDevice, pContext, pArg)))
	{
		MSG_BOX("Make Failed :CUISkillDestroyer");

...
CUISkillRush::CUISkillRush()
	:CUIScene{}
{
}

CUISkillRush::CUISkillRush(const CUISkillRush& rhs)
	:CUIScene{ rhs }
{
}
...
HRESULT CUISkillRush::Show_Scene()
{
	__super::Show_Scene();
...
HRESULT CUISkillRush::Close_Scene()
{
	if (m_isGet)
	{
...
HRESULT CUISkillRush::Initialize(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, void* pArg)
{
...
HRESULT CUISkillRush::Tick(const _float& fTimeDelta)
{
...
HRESULT CUISkillRush::Late_Tick(const _float& fTimeDelta)
{
	for (size_t i = 0; i < m_UI.size(); i++)
	{
		if (m_fGetTime > 0.f)
...
_bool CUISkillRush::Click_InterSect()
{
	for (size_t i = 0; i < m_pBall.size(); i++)
	{
...
_bool CUISkillRush::Over_InterSect()
{
	for (size_t i = 0; i < m_pBall.size(); i++)
	{
		if (m_pBall[i]->Click_Intersect(0))
...
void CUISkillRush::Set_Money(const _float& fTimeDelta)
{
...
CUISkillRush* CUISkillRush::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, void* pArg)
{
	CUISkillRush* pInstance = new CUISkillRush();
	if (FAILED(pInstance->Initialize(pDevice, pContext, pArg)))
	{
		MSG_BOX("Make Failed :CUISkillRush");

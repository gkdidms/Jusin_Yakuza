...
HRESULT CLevel_Office1F::Initialize()
{
	...
	m_pFileTotalManager->Set_Collider_In_Client(1, LEVEL_OFFICE_1F);
...
HRESULT CLevel_Office1F::Ready_Camera(const wstring& strLayerTag)
{
	...
	if (FAILED(m_pGameInstance->Add_GameObject(LEVEL_OFFICE_1F, TEXT("Prototype_GameObject_DebugCamera"), strLayerTag, &CameraDesc)))
		return E_FAIL;
	...
	cineDesc.iFileNum = -1;
	if (FAILED(m_pGameInstance->Add_GameObject(LEVEL_OFFICE_1F, TEXT("Prototype_GameObject_CCineCamera"), strLayerTag, &cineDesc)))
		return E_FAIL;
...
HRESULT CLevel_Office1F::Ready_Player(const wstring& strLayerTag)
{
	...
	if (FAILED(m_pGameInstance->Add_GameObject(LEVEL_OFFICE_1F, TEXT("Prototype_GameObject_Player"), strLayerTag, &Desc)))
		return E_FAIL;

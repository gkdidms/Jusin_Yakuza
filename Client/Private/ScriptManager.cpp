...
void CScriptManager::Ready_Script_1()
{
	...
	Scrpit_1.emplace(iIndex++, SCRIPT_INFO(TEXT("사채업자"), TEXT("동년배 꼬맹이들이 만 엔 지폐 뿌리면서 택시 잡는 시대에, 현역인 자네가 돈도 없으면 모양이 안 나잖아. 뭐, 이건 나하고의 첫 일을 끝낸 기념 같은 거야. ...받아줘.")));
	Scrpit_1.emplace(iIndex++, SCRIPT_INFO(TEXT("키류"), TEXT("...감사합니다.")));
	Scrpit_1.emplace(iIndex++, SCRIPT_INFO(TEXT("사채업자"), TEXT("훗, 그런데 바보한테 돈 빌려주면 뒷일이 너무 귀찮아진다니까. 30만엔 따위 그냥 무시해도 상관은 없지만. 회수해두지 않으면 장사꾼으로서 무시당할 수도 있으니까... 안 그런가?")));
	Scrpit_1.emplace(iIndex++, SCRIPT_INFO(TEXT("키류"), TEXT("...예, ...뭐.")));
	Scrpit_1.emplace(iIndex++, SCRIPT_INFO(TEXT("사채업자"), TEXT("나도 이 동네에서 꽤 오랫동안 사채꾼을 하고 있으니 잘알지... 요즘 세상에 돈의 진짜 가치를 알고 있는 놈은 거의 없어. 오히려 배운 게 없는 자네 쪽이 고학력의 애송이보다 훨씬 알고 있는 느낌이 드는데. 어때?")));
	Scrpit_1.emplace(iIndex++, SCRIPT_INFO(TEXT("키류"), TEXT("저는 잘 모르겠습니다. ...그럼 이만.")));
	Scrpit_1.emplace(iIndex++, SCRIPT_INFO(TEXT("사채업자"), TEXT("이봐. 같이 일해보지 않겠나 ...형씨. 자네 실력이면 야쿠자 하는 것보다 훨씬 간단히 더 많은 돈을 벌 수 있어.좋은 밥 배불리 먹고 예쁜 여자도 마음껏 안을 수 있지.그뿐만이 아냐.돈만 있으면.불행한 인간을 행복하게 만들어 줄 수도 있어.야쿠자는 밑바닥이 길어....더 빨리 출세하고 싶다면, 이쪽에 와서 돈을 버는 것도 좋은 선택이라고 생각하는데 ? ")));
	Scrpit_1.emplace(iIndex++, SCRIPT_INFO(TEXT("키류"), TEXT("3년 전에 그 말을 들었으면 그렇게 했을지도 모르겠군요. 하지만 한 번 건달이 된 이상... 시간을 되돌릴 수는 없습니다.")));
	...
	Scrpit_1.emplace(iIndex++, SCRIPT_INFO(TEXT("사채업자"), TEXT("훗, 글쎄.... 난 야쿠자가 된 적 없어서 잘 모르겠다. 뭐, 생각이 바뀌면 언제든 연락해. ...그럼.")));
...
void CScriptManager::Ready_Script_2()
{
	...
	_int iIndex = 0;
	Scrpit.emplace(iIndex++, SCRIPT_INFO(TEXT("키류"), TEXT("(...니시키와 만나기로 했던 곳, 이 근처일 텐데 녀석은 아직 안온 건가 ? [724106(뭐하고 있어 ? )]라고 하는 주제에....할 수 없지.잠깐 니시키를 찾으러 다녀볼까.)")));

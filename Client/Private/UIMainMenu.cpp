...
void CUIMainMenu::Action()
{
	...
	{
		// 레벨지정
		if (FAILED(m_pGameInstance->Open_Level(LEVEL_LOADING, CLevel_Loading::Create(m_pDevice, m_pContext, LEVEL_CARCHASE))))
			return;
		break;
	}
	case 2:
	{
		if (FAILED(m_pGameInstance->Open_Level(LEVEL_LOADING, CLevel_Loading::Create(m_pDevice, m_pContext, LEVEL_TOKOSTREET))))
			return;
		break;
	}
	default:

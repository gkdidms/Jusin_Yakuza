...
float3 SubSucface(float2 vTexcoord, float4 vNormal)
{
    ...
    float vScatterAmount = saturate(dot(normalize(g_vLightDir), vNormal)); // 빛의 산란값
...
float3 CalcuateSpecular(float2 vTexcoord, float4 vNormal, float3 vLook)
{
    ...
    float spec = pow(max(dot(reflectDir, vLook), 0.0), vOEShader.x); // 스펙큘러 계산
    return spec * vSpecular.r; // 스펙큘러 강도 반환
}
...
void GS_MAIN(uint3 dispatchThreadID : SV_DispatchThreadID)
{
    ...
    minDepth = min(minDepth, InputTexture.Load(int3(outputCoord * 2 + uint2(0, 0), 0)));
    minDepth = min(minDepth, InputTexture.Load(int3(outputCoord * 2 + uint2(1, 0), 0)));
    minDepth = min(minDepth, InputTexture.Load(int3(outputCoord * 2 + uint2(0, 1), 0)));
    minDepth = min(minDepth, InputTexture.Load(int3(outputCoord * 2 + uint2(1, 1), 0)));
